...
import {
  FolderKanban, CheckCircle2, FileEdit, Archive, Star, IndianRupee,
} from 'lucide-react'
...
export default function ProjectStatsCards({ stats, loading }) {
  if (loading) {
    return (
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      {cards.map((card, i) => {
        ...
        const display = card.prefix ? `${card.prefix}${animateValue(val)}` : animateValue(val)
        ...
        return (
          ...
          >
            <div className={`mb-4 flex h-10 w-10 items-center justify-center rounded-btn bg-gradient-to-br ${card.tint} transition-all duration-300 group-hover:shadow-[0_0_24px_var(--glow)]`}
